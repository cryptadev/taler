...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (pblock) {
            int legacy_flag = (pfrom->IsLegacyBlockHeader(pfrom->GetSendVersion()) ? SERIALIZE_BLOCK_LEGACY : 0);
            if (inv.type == MSG_BLOCK)
                connman->PushMessage(pfrom, msgMaker.Make(legacy_flag | SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::BLOCK, *pblock));
            else if (inv.type == MSG_WITNESS_BLOCK)
                connman->PushMessage(pfrom, msgMaker.Make(legacy_flag, NetMsgType::BLOCK, *pblock));
            else if (inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (sendMerkleBlock) {
                    connman->PushMessage(pfrom, msgMaker.Make(legacy_flag, NetMsgType::MERKLEBLOCK, merkleBlock));
                    ...
                    for (PairType& pair : merkleBlock.vMatchedTxn)
                        connman->PushMessage(pfrom, msgMaker.Make(legacy_flag | SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::TX, *pblock->vtx[pair.first]));
                }
                // else
                    // no response
            }
