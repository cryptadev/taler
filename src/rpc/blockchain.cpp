...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("proofhash", blockindex->IsProofOfStake() ? blockindex->GetStakeHash().GetHex() : blockindex->GetBlockHeader().GetPoWHash(blockindex->nHeight, Params().GetConsensus()).GetHex());
    result.pushKV("entropybit", (int)blockindex->GetStakeEntropyBit());
    result.pushKV("modifier", strprintf("%016llx", blockindex->GetStakeModifier()));
    return result;
}
