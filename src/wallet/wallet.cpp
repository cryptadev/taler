...
bool getCoinInfo (std::map<uint256, uint64_t>& cache, const COutPoint& out, uint32_t& time, uint32_t& offset) {
    if (!cache.count(out.hash)) {
        ...
        if (pcoinsTip->GetCoin(out, coin) && (chainActive[coin.nHeight] != nullptr)) {
            ...
            correctCoin (out, coin, "getCoinInfo", time, offset);
            uint64_t tmp = offset; tmp <<= 32; tmp |= time; 
            cache[out.hash] = tmp;
        }
        if (!cache.count(out.hash)) return false;
    }
    uint64_t tmp = cache[out.hash];
    time = tmp & 0xFFFFFFFF;
    offset = (tmp >> 32); 
    return true;
};
...
bool CWallet::CreateCoinStake (CBlockHeader& header, int64_t nSearchInterval, CMutableTransaction &txNew, CAmount& nPosReward)
{
    ...
    for (const COutput& inpcoin : vCoins) {
        ...
        for (uint32_t n = 0; n < nSearchInterval && !fKernelFound; n++) {
            ...
            if (CheckProofOfStake (header, pPrev->nHeight + 1, consensus, pcoin.outpoint, pcoin.txout.nValue, time, offset)) {
                ...
                if (!ret) {
                    return error("CreateCoinStake: Keypool ran out, please call keypoolrefill first");
                }

                LearnRelatedScripts(vchPubKey, DEFAULT_ADDRESS_TYPE);
                CScript scriptPubKeyOut = GetScriptForDestination(
                    GetDestinationForKey(vchPubKey, DEFAULT_ADDRESS_TYPE));
    ...
    {
        // The first output MUST be "pay to public key" (in this case zero-valued, but it's not important).
        // Public key from first output will be used into SignBlock.
        CPubKey vchPubKey;
        bool ret = key0.GetReservedKey(vchPubKey, true);
        if (!ret) {
            return error("CreateCoinStake: Keypool ran out, please call keypoolrefill first");
        }
        CScript scriptPubKeyCoinstakeBlockKey;
        scriptPubKeyCoinstakeBlockKey << ToByteVector(vchPubKey) << OP_CHECKSIG;
        txNew.vout.insert(txNew.vout.begin(), CTxOut(0, scriptPubKeyCoinstakeBlockKey));
    }
